...
const UpcomingRaceSchedulePage = async ({ params }) => {
  ...
  const race = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/schedule/season/current/${params.round}`, { cache: 'no store'}
  ).then((res) => res.json())

  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/tracks/${race.Circuit.circuitId}`, { cache: 'no store'})
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center bg-background">
      ...
      <div className="w-full lg:w-9/12 2xl:w-2/3 flex flex-col-reverse gap-y-16 justify-evenly pt-2 bg-contentBackground md:flex-row">
        {/* track layout image */}
        {/* <div className="pt-8 pl-8"> */}
        <div className="flex items-center pl-4 md:pl-8">
          <div className="flex flex-row items-center gap-x-2">
            <div className="overflow-hidden relative border-solid border-2 border-contentColor w-16 md:w-24 h-10">
              <Image
      ...
      <div className="w-full lg:w-9/12 2xl:w-2/3 px-4 md:px-8 pt-4 md:pt-8 h-auto bg-contentBackground flex flex-col md:flex-row">
