      ...
      {races &&
        races.map((race, index) => {
          return (
            <Link
              key={index}
              className={`bg-gray-300 dark:bg-slate-700 hover:bg-gray-400 w-9/10 border-8 border-solid rounded-md border-gray-300 dark:border-slate-700 ${
                index === 0 ? "hidden" : ""
              }`}
              href={`schedule/current/completed/${race.round}`}
            >
              <div className="flex flex-row px-2 items-center">
                ...
                <div className="overflow-hidden relative border-solid border-gray-300 rounded-md border-2 w-16 md:w-24 h-10">
                  ...
                  <Image
                    src={`/assets/images/flag/${race.Circuit.Location.country.replace(
                      / /g,
                      "_"
                    )}_flag.svg`}
                ...
                <div className="text-l md:text-xl text-right basis-1/4">
                  {race.FirstPractice.date.split("-").slice(1).join("/")} -{" "}
                  {race.date.slice(8)}
                </div>
              </div>
            </Link>
