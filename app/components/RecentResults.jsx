...
const RecentResults = async ({ raceRound, race }) => {
  const qualifyingResponse = await fetch(`http://ergast.com/api/f1/current/${raceRound}/qualifying.json`, { next: {revalidate: 360000}})
  const qualifyingData = await qualifyingResponse.json()
  const qualifyingResults = qualifyingData.MRData.RaceTable.Races

  const raceResultsResponse = await fetch(`http://ergast.com/api/f1/current/${raceRound}/results.json`, { next: {revalidate: 360000}})
