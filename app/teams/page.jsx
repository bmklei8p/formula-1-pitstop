...
const TeamsPage = () => {
  ...
  useEffect(() => {
    const getConstructorsStandings = async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/standings/constructors`,
        { next: { revalidate: 60 } }
      );
  ...
  useEffect(() => {
    const getConstructorsInformation = async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/constructors/mdb`,
        { next: { revalidate: 60 } }
      );
      const data = await res.json();
    ...
    <div className="w-full mx-4 mt-4 md:w-3/4">
      ...
      <div className="grid grid-cols-1 mx-3 gap-8 lg:grid-cols-2 lg:gap-16">
        ...
        {constructorsStandings &&
          constructorsInformation &&
          constructorsStandings.map((constructor, index) => {
            const constructorId = constructor.Constructor.constructorId;
            const constructorCarURL =
              constructorsInformation.get(constructorId)["carURL"];
            const driverOne =
              constructorsInformation.get(constructorId)["driverOne"];
            const driverOneFirst = driverOne.split(" ")[0];
            const driverOneLast = driverOne.split(" ")[1];
            const driverTwo =
              constructorsInformation.get(constructorId)["driverTwo"];
            const driverTwoFirst = driverTwo.split(" ")[0];
            const driverTwoLast =
              driverTwoFirst === "Nyck" ? "de Vries" : driverTwo.split(" ")[1];
            ...
            return (
              <Link key={index} href={`teams/${constructorId}`}>
                ...
                <div
                  className="hidden border-[#3471c5] border-[#6cd3be] border-[#358c75] border-[#f91435] border-[#f5801f]
          ...
          hover:border-[#2293d2] hover:border-[#37bedd] hover:border-[#b6babd] hover:border-[#c92d4b] hover:border-[#5e8faa]"
                ...
                <div
                  className={`flex p-2 w-full flex-col border-t-2 border-r-2 border-borderColor rounded-tr-lg hover:border-[${color}] hover:border-t-4 hover:border-r-4 `}
                >
                  <div className="flex flex-row justify-between">
                    <div className="flex justify-center items-center font-bold text-3xl md:text-4xl">
                      <h1>{index + 1}</h1>
                    </div>
                  ...
                  <div
                    className={`flex flex-row pt-2 pb-2 items-center justify-between border-t-2 border-b-2 border-borderColor`}
                  >
                    <div
                      className={`border-l-4 border-solid border-[${color}]`}
                    >
                      <h3 className="px-2 text-xl md:text-2xl font-bold">
                        {constructor.Constructor.name}
                      </h3>
                    </div>
                    <div className=" overflow-hidden relative w-12 h-12">
                      <Image
                        src={`/assets/images/logo/${constructorId}_logo.avif`}
                  ...
                  <div className="flex flex-row w-full pt-2 pb-2 items-center gap-4">
                    <div
                      className={`basis-1/2 border-r-2 border-b-2 border-solid border-borderColor rounded-br-lg `}
                    >
                      {/* split driver on two rows here- next */}
                      <div className="flex flex-col md:flex-row md:gap-1">
                        <h3 className="text-lg">{driverOneFirst}</h3>
                        <strong className="text-lg">{driverOneLast}</strong>
                      </div>
                    </div>
                    <div
                      className={`basis-1/2 border-r-2 border-b-2 border-solid border-borderColor rounded-br-lg `}
                    >
                      {/* split driver on two rows here- next */}
                      <div className="flex flex-col md:flex-row md:gap-1">
                        <h3 className="text-lg">{driverTwoFirst}</h3>
                        <strong className="text-lg">{driverTwoLast}</strong>
                      </div>
                    </div>
                  </div>
                  <div className="w-full flex justify-center">
                    <div className="overflow-hidden relative w-full lg:w-11/12 h-24 md:h-32 ">
                      <Image
                        className="object-contain"
                        src={`/assets/images/cars/${constructorCarURL}`}
                        sizes="(max-width: 768px) 100vw, (max-width: 1200px) 75vw"
                        alt={`${constructor.Constructor.name}'s car`}
                        fill={true}
                      />
                    </div>
                  </div>
