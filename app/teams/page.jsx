...
const TeamsPage = () => {
  ...
  useEffect(() => {
    const getConstructorsStandings = async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/standings/constructors`, { cache: 'no-store'})
  ...
  useEffect(() => {
    const getConstructorsInformation = async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/constructors/mdb`, { cache: 'no-store'})
      const data = await res.json()
