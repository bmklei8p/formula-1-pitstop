...
const DriverSingleRaceStandingsTable = ({standings}) => {
  return (
      <div className='xl:w-1/2 w-full sm:pt-6 overflow-y-hidden'>
          {/* table header */}
          <div className="bg-slate-300 dark:bg-slate-800 px-6 py-4 w-1/10 max-w-xl mx-auto shadow-lg flex items-start border-solid border-red-500 border-b-[3px]">
              ...
              <div className="w-full basis-3/5 md:flex-row">
                  <div className="text-xl font-medium">Finish</div>
              </div>
              <p className='font-medium text-lg basis-1/5'>Points</p>
          </div>
          {/* map through standings and display each position */}
          {standings.map((pos) => (
              <div key={pos.position} className="px-6 py-4 max-w-xl mx-auto shadow-lg flex items-start even:bg-slate-200 dark:even:bg-slate-700 dark:odd:bg-slate-800 odd:bg-slate-300">
                  ...
                  <Link href={`/drivers/${pos.Driver.driverId}`} className="w-full md:flex-row basis-3/5">
                      <div className="text-l md:text-xl">{pos.Driver.givenName + " " + pos.Driver.familyName}</div>
                  </Link>
                  <div className="w-full basis-2/5 md:flex-row">
                    <div className="text-l font-medium ">{pos.status === "Finished"  ? pos.Time.time : (pos.status.includes('Lap') ? pos.status : "DNF") } </div>
                  </div>
