...
const StandingsPage = ({showContructors}) => {
  ...
  useEffect(() => {
    const getDriverStandings = async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/standings/drivers`, { next: {revalidate: 360000}}
      );
  ...
  useEffect(() => {
    const getConstructorsStandings = async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/standings/constructors`, { next: {revalidate: 360000}}
      );
      const data = await res.json();
  ...
  return (
    <div className="standings-container">
      <h1 className="hidden text-4xl font-bold mb-4 md:block">
        {showDriversStandings ? "Drivers Standings" : "Constructors Standings"}
      </h1>
      {/* mobile buttons */}
      <div className="flex justify-center md:hidden">
        <div
          className="basis-1/2"
          onClick={() => setShowDriversStandings(true)}
        >
          <button
            className={`${
              showDriversStandings
                ? `text-2xl border-b-4 w-full border-red-500 rounded`
                : `text-xl`
            }`}
          >
            Drivers
        ...
        <div
          className="basis-1/2"
          onClick={() => setShowDriversStandings(false)}
        >
          <button
            className={`${
              !showDriversStandings
      ...
      <div
        className={`flex flex-col-reverse justify-center ${
          showDriversStandings
            ...
            : `md:flex-row-reverse md:align-center`
        }`}
      >
        {/* show appropriate table based on showDriversStandings */}
        {showDriversStandings ? (
          <DriverStandingsTable standings={driversStandings} />
        ) : (
          <ConstructorStandingsTable standings={constructorsStandings} />
        )}

        {/* show appropriate podium based on showDriversStandings */}
        {firstPlaceC && firstPlaceD && showDriversStandings ? (
          <DriverPodium
            firstPlace={firstPlaceD}
            secondPlace={secondPlaceD}
            thirdPlace={thirdPlaceD}
          />
        ) : (
          <ConstructorPodium
            firstPlace={firstPlaceC}
            secondPlace={secondPlaceC}
            thirdPlace={thirdPlaceC}
          />
        )}
      </div>
    </div>
